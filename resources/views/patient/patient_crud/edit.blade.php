...
@section('content')
            ...
            <main class="main-content">
                ...
                <div class="card">
                    <form class="p-4" action="{{ route('patient.patient_crud.update', $appointment->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <select id="appointmentType" name="appointmentType" class="form-select" required>
                            ...
                            @foreach ($appointmentTypes as $type)
                                <option value="{{ $type->id }}" {{ $type->id == $appointment->appointmentType->id ? 'selected' : '' }}>{{ $type->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="doctor" name="doctor" class="form-select" required>
                            ...
                            @foreach ($doctors as $doctor)
                                <option value="{{ $doctor->id }}" {{ $doctor->id == $appointment->doctor->id ? 'selected' : '' }}>{{ $doctor->user->first_name }} {{ $doctor->user->last_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="date" name="date" class="form-control" value="{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('Y-m-d') }}" required>
                    </div>

                    <!-- Time Selection -->
                    <div class="form-group">
                        <label for="time" class="form-label">Preferred Time</label>
                        <input type="time" id="time" name="time" class="form-control" value="{{ \Carbon\Carbon::parse($appointment->appointment_time)->format('H:i') }}" required>
                    </div>

                    <!-- Reason for Visit -->
                    <div class="form-group">
                        <label for="reason" class="form-label">Reason for Visit</label>
                        <textarea id="reason" name="reason" class="form-textarea" placeholder="Please describe your symptoms or reason for the appointment" required>{{ $appointment->reason }}</textarea>
                    </div>

                    <!-- Additional Notes -->
                    <div class="form-group">
                        <label for="notes" class="form-label">Additional Notes (Optional)</label>
                        <textarea id="notes" name="notes" class="form-textarea" placeholder="Any additional information you'd like to share">{{ $appointment->notes }}</textarea>
                    </div>
